'''
This functin is to test models on the binned data. Similar to the parm_signif_tests.py
in the Main folder, the function tests every combination of the eight parameters.
The results are used to generate Fig. 2b in the paper.
...
from addAttribs_binned import addAttribs
...
#######################    set up model path  #######################
PROJECT_ROOT_DIR = ''
MODEL_PATH = os.path.join(PROJECT_ROOT_DIR,"binned_Output")

if not os.path.isdir(MODEL_PATH):
    os.mkdir(MODEL_PATH)

# path and filename of saved model
saved_model_path = MODEL_PATH + "/binned_combs3-2.h5"
scaler_filename1 = MODEL_PATH + "/binned_scaler_combs3-2.save"
#######################    set up model path  #######################
...
def load_DMS_data(DMS_path=DMS_PATH):
    csv_path=os.path.join(DMS_path,"PMEL_NAMMES_binned_DMS_Envi.csv")
...
rm_cols1 = ["MON", "latitude","longitude", "Kd490","SRD"]
val_col = ["MLD","SST","SSS","PAR","SiO","PO4","NO3"]#,"Chl"
ErrorData = pd.DataFrame() # DataFrame for storing errors
numCom = 2 # number of environmental parameters in each combination
ErrorFile = '/ErrorCombs'+str(numCom+1)+'-2.csv'
...
for rm_cols2 in combs:
    ...
    junk = ['latlon1', 'latlon2', 'latlon3', 'MON1', 'MON2']
